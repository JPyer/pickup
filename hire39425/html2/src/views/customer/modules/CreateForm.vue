  ...
  >
    <a-spin :spinning="loading">
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item v-show="model && model.device_id > 0" label="主键ID">
          <a-input v-decorator="['device_id', { initialValue: 0 }]" disabled />
        </a-form-item>
        <a-form-item label="设备名称">
          <a-input v-decorator="['name', {rules:[{required: false, message: '请输入设备名称'}]}]" />
        </a-form-item>
        <a-form-item label="设备ip">
          <a-input v-decorator="['ip_addr', {rules:[{required: false, message: '请输入ip地址'}]}]" />
        </a-form-item>
        <a-form-item label="网卡物理地址">
        ...
        <a-form-item label="备注">
          <a-input v-decorator="['remark', {rules: [{required: false, min: 5, message: '请输入至少五个字符的规则描述！'}]}]" />
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
...
// 表单字段
const fields = ['device_id', 'name', 'device_group_id', 'mac_addr', 'ip_addr', 'subnet', 'gateway', 'dns_server1', 'dns_server2', 'security_key', 'remark']
